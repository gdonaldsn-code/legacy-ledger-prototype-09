-- Rewards/points accounts — the last of the three RewardsPerks tabs still
-- backed by a hardcoded fixture instead of real per-user rows. Same pattern
-- as contacts/legal_documents: RLS-scoped table, seeded at signup.

create table public.rewards_accounts (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  name text not null,
  type text not null check (type in ('airline', 'hotel', 'credit-card', 'retail')),
  program text not null,
  balance text not null,
  estimated_value numeric(10, 2) not null default 0,
  expiration_warning text,
  monthly_perks text[] not null default '{}',
  created_at timestamptz not null default now()
);

alter table public.rewards_accounts enable row level security;

create policy "rewards_accounts_select_own" on public.rewards_accounts
  for select using (auth.uid() = user_id);
create policy "rewards_accounts_insert_own" on public.rewards_accounts
  for insert with check (auth.uid() = user_id);
create policy "rewards_accounts_update_own" on public.rewards_accounts
  for update using (auth.uid() = user_id);
create policy "rewards_accounts_delete_own" on public.rewards_accounts
  for delete using (auth.uid() = user_id);

-- Extend the starter-data seed to include rewards accounts too.
...
begin
  ...
  insert into public.rewards_accounts (user_id, name, type, program, balance, estimated_value, expiration_warning, monthly_perks)
  values
    (new.id, 'American Airlines', 'airline', 'AAdvantage', '47,500 miles', 712,
      null, array['Priority boarding', 'Free checked bag', 'Admiral''s Club access (2 passes/month)']),
    (new.id, 'Delta Air Lines', 'airline', 'SkyMiles', '32,100 miles', 481,
      'Miles expire in 18 months without activity', array['Sky Priority check-in', 'Complimentary upgrades when available']),
    (new.id, 'Marriott Bonvoy', 'hotel', 'Bonvoy Gold Elite', '89,200 points', 623,
      null, array['25% bonus points on stays', 'Room upgrade when available', 'Late checkout (2pm)', 'Welcome gift points']),
    (new.id, 'Chase Sapphire Reserve', 'credit-card', 'Ultimate Rewards', '125,000 points', 1875,
      null, array['$300 annual travel credit', 'Priority Pass lounge access', 'DoorDash DashPass membership', '$100 Global Entry/TSA credit']),
    (new.id, 'Hilton Honors', 'hotel', 'Diamond Status', '156,800 points', 784,
      null, array['Free breakfast', 'Executive lounge access', '5th night free on award stays', 'Room upgrade to suites']),
    (new.id, 'Amazon Prime Rewards', 'retail', 'Prime Visa', '18,450 points', 184,
      null, array['5% back on Amazon purchases', '2% back at restaurants & gas stations', 'No foreign transaction fees']);
